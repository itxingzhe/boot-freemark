function initializeBmap(dom, center) {
    // 百度地图API功能
    window.map = new BMap.Map(dom, {enableMapClick: false});    // 创建Map实例
    //map.centerAndZoom(new BMap.Point(119.30495233371025, 26.105880034475164), 11);  // 初始化地图,设置中心点坐标和地图级别
    map.centerAndZoom(new BMap.Point(center[0], center[1]), 11);  // 初始化地图,设置中心点坐标和地图级别
    ...
    map.setMapStyleV({styleJson: styleJson});
}

function localBmapScript() {
    var script = window.document.createElement("script");
    script.type = "text/javascript";
    script.src = "http://api.map.baidu.com/api?v=2.0&ak=M2iMmqRqUj7ZeonOfFfpqg0SHl4VQG3Q&callback=initializeBmap('allmap',[120.131126, 30.265072])";
